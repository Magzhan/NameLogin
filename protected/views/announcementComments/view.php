<?php
/* @var $this AnnouncementCommentsController */
/* @var $model AnnouncementComments */

$this->breadcrumbs=array(
	'Announcement Comments'=>array('index'),
	$model->id,
);

$this->menu=array(
	array('label'=>'List AnnouncementComments', 'url'=>array('index')),
	array('label'=>'Create AnnouncementComments', 'url'=>array('create')),
	array('label'=>'Update AnnouncementComments', 'url'=>array('update', 'id'=>$model->id)),
	array('label'=>'Delete AnnouncementComments', 'url'=>'#', 'linkOptions'=>array('submit'=>array('delete','id'=>$model->id),'confirm'=>'Are you sure you want to delete this item?')),
	array('label'=>'Manage AnnouncementComments', 'url'=>array('admin')),
);
?>

<h1>View AnnouncementComments #<?php echo $model->id; ?></h1>

<?php $this->widget('zii.widgets.CDetailView', array(
	'data'=>$model,
	'attributes'=>array(
		'id',
		'content',
		'DateCreated',
		'LastUpdate',
		'users_id',
		'announcement_id',
	),
)); ?>
